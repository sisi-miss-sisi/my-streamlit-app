                    ...
                    if st.secrets.get("DEEPSEEK_API_KEY"):
                        try:
                            ...
                            user_prompt = f"""
                                   ...
                                   请根据以上完整的血糖记录，用一段话（不超过200字）向医生描述用户近期的血糖变化趋势。只需要客观陈述最大最小值、备注中不同饮食以及餐前餐后所反映的血糖上升下降趋势变化或平均值等。不要进行任何标准对比（如评价这段时间数值较高），也不要给出“正常/不正常”或“好/不好”或“高/较高”的评价。
                                   """
                    ...
                    if st.secrets.get("DEEPSEEK_API_KEY"):
                        try:
                            ...
                            user_prompt = f"""
                                   ...
                                   请根据以上完整的血糖记录，用一段话（不超过200字）向医生描述用户近期的血压变化趋势。只需要客观陈述高低压的最大最小值以及数值上升下降趋势变化或平均值等。不要进行任何标准对比（如评价这段时间数值较高），也不要给出“正常/不正常”或“好/不好”或“高/较高”的评价。
                                   """
                            response = client.chat.completions.create(
                                model="deepseek-chat",
                                messages=[
